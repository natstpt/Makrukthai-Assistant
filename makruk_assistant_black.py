...
class TransparentGreenDotOverlay(QWidget):
    ...
    def calculate_centers(self):
        ...
        source_x, source_y = 7 - \
            (ord(self.source_square[0]) - ord('a')
             ), int(self.source_square[1]) - 1
        self.source_center_x, self.source_center_y = x0 + \
            source_x * x_offset, y0 + source_y * y_offset

        destination_x, destination_y = 7 - \
            (ord(self.destination_square[0]) - ord('a')
             ), int(self.destination_square[1]) - 1
...
def main():
    ...
    # Populate the board array for the black side's perspective
    for match in reversed(matches):
        pt, name = match
        row, col = 7 - pt[1] // space_size, 7 - pt[0] // space_size
        board[row][col] = name

    # Convert the board array to a FEN string with columns ordered from a to h
    fen = ''
    for row in board:
        empty = 0
        for cell in row:
    ...
    # Remove the trailing '/' and add 'b' to indicate it's black's turn
    fen = fen[:-1] + ' b'
    ...
    """ # send the command to set the SyzygyPath EGTB
